		...
		.on('change', function (e) {
			...
			$.ajax({
				...
				.done((data) => {
					...
					const messageOfMe = $(
						`<div class="me bubble bubble-image-file" data-mess-id="${_id}"></div>`
					);
					const imageHtml = `<img src="data:${
						file.contentType
					}; base64, ${bufferToBase64(
						file.data.data
					)}" class="show-image-chat" title="${new Date(
						createdAt
					).toLocaleString()}">`;
					...
					if (isChatGroup) {
						...
						messageOfMe.html(
							`<img src="./libraries/images/users/${sender.avatar}" class="avatar-small" title="${sender.name}" /> ${imageHtml}`
						);
					...
					userLeft
						.find('.time')
						.text(moment(createdAt).locale('vi').startOf('seconds').fromNow());
					userLeft
						.find('.preview')
						.html('<i class="fa fa-file-image-o"></i> Hình ảnh');
					...
					const imageHtmlModal = `<img src="data:${
						file.contentType
					}; base64, ${bufferToBase64(file.data.data)}">`;
...
socket.on('response-add-new-image-message', (response) => {
	...
	const messageOfYou = $(
		`<div class="you bubble bubble-image-file" data-mess-id="${_id}"></div>`
	);
	const imageHtml = `<img src="data:${
		file.contentType
	}; base64, ${bufferToBase64(
		file.data.data
	)}" class="show-image-chat" title="${new Date(createdAt).toLocaleString()}">`;
